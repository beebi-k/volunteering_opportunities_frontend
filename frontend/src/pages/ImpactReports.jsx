...
import { Download, Share2, TrendingUp, Award, Heart, Users, X, Copy } from 'lucide-react';
...
const ImpactReports = () => {
  ...
  const handleDownload = () => {
    const doc = new jsPDF();
    doc.setFontSize(18);
    doc.text('Volunteer Impact Report', 20, 20);

    doc.setFontSize(12);
    doc.text('Monthly Contribution:', 20, 40);

    let y = 50;
    data.forEach(item => {
      doc.text(`${item.month}: ${item.hours} hours`, 20, y);
      y += 8;
    });

    doc.text('Category Distribution:', 20, y + 10);
    y += 20;
  ...
  // ✅ SHARE FUNCTIONS
  const shareWhatsApp = () => {
    window.open(`https://wa.me/?text=Check%20my%20impact%20report:%20${pageUrl}`, '_blank');
  };
    ...
    <div className="flex h-full flex-col bg-white dark:bg-zinc-950">
      <header className="flex items-center justify-between border-b border-black/5 px-8 py-6 dark:border-white/10">
        ...
        <div className="flex gap-3">
          <button 
            ...
            className="flex items-center gap-2 rounded-xl border border-black/5 bg-white px-4 py-2 text-sm font-bold text-zinc-900 shadow-sm hover:bg-zinc-50 dark:border-white/10 dark:bg-zinc-800 dark:text-white"
          >
          ...
          <button 
            ...
            className="flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-bold text-white shadow-lg shadow-emerald-600/20 hover:bg-emerald-700"
          >
      ...
      {showShare && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-80 rounded-2xl bg-white p-6 shadow-xl dark:bg-zinc-900">
            ...
            <div className="space-y-3">
              <button onClick={shareWhatsApp} className="w-full rounded-xl bg-green-500 py-2 text-white font-bold">
                Share on WhatsApp
              </button>
              <button onClick={shareFacebook} className="w-full rounded-xl bg-blue-600 py-2 text-white font-bold">
                Share on Facebook
              </button>
              <button onClick={shareInstagram} className="w-full rounded-xl bg-pink-500 py-2 text-white font-bold">
                Share on Instagram
              </button>
              <button onClick={copyLink} className="w-full rounded-xl bg-zinc-800 py-2 text-white font-bold flex items-center justify-center gap-2">
