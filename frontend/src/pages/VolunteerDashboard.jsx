...
const VolunteerDashboard = () => {
  ...
  // ✅ ADDED DOWNLOAD FUNCTION (Nothing else changed)
...
Hours By Month:
${stats.hoursByMonth.map(m => `${m.month}: ${m.hours} hours`).join('\n')}

Impact By Category:
${stats.hoursByCategory.map(c => `${c.name}: ${c.value} hours`).join('\n')}

Badges:
${stats.badges.map(b => `${b.name} - ${b.earned ? 'Unlocked' : 'Locked'}`).join('\n')}
    `;
    ...
    link.href = url;
    link.setAttribute('download', 'Volunteer_Impact_Report.txt');
